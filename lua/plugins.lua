...
return require('packer').startup{ function()
	...
	use { '3lv/femboyscheme', requires = { 'rktjmp/lush.nvim' }, config = 'vim.cmd[[colorscheme femscheme]]' }
	use { '3lv/femboyalt',                   config = function() require('femboyalt') end }
	use { '3lv/femboycpp', ft = {'cpp'} }
	...
	use { 'kyazdani42/nvim-tree.lua', event = 'BufRead', config = function() require('config.nvim-tree') end }
	use { 'nvim-telescope/telescope.nvim', opt = true, requires = { { 'nvim-lua/popup.nvim', opt = true }, { 'nvim-lua/plenary.nvim', opt = true }}, config = function() require('config.telescope') end }
	use { 'nvim-treesitter/nvim-treesitter', config = function() require('config.nvim-treesitter') end }
	use { 'neovim/nvim-lspconfig', opt = true, config = function() require('config.lsp') end }
	use { 'glepnir/lspsaga.nvim', opt = true, config = function() require('config.lspsaga') end }
